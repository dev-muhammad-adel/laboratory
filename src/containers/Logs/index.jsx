...
const Countries = () => {
  ...
  const deleteRecord = ({ id }) => {
    toast.promise(
      ...
      }),
      {
        ...
        error: {
          render(err) {
            return `🤯 ${err?.data?.response?.data?.message || "Backend Error Occured"
              }`;
          },
          // other options
          //    icon: "🟢",
  ...
  const filters = [
    ...
    //       { id: "Patient", ["name_en"]: "Patient" },
    //       { id: "Partner", ["name_en"]: "Partner" },
    //     ],
    //     optionName: "name_en",
    //   },
    // },
  ...
  return (
    <>
      <IndexMain
        ...
        // head table content
        addRowURL={getPermissions("pages", "create") ? config.routes.add : false}
        // head filters table
        filters={filters}
        // columns table content
        editRowURL={getPermissions("pages", "update") ? config.routes.edit : false}
        // showRowURL={config.routes.show}
        deleteRow={{ status: getPermissions("pages", "delete"), deleteRecord }}
        generalCols={generateCols([
          "actor_name",
          {
            title: "Actor IP",
            key: "ip",
            content: (text, record) => <a target="_blank" href={`https://ipgetinfo.com/?mode=ip&lang=en&ip=${text}`}>{text}</a>
          },
          {
            title: "Action",
            key: "subject",
            content: (text, record) => <div>{text}</div>
          },
          {
            title: "Target",
            key: "object_name",
        ...
        options={[]}
        action={(getPermissions("pages", "delete") || getPermissions("pages", "update")) || false}
