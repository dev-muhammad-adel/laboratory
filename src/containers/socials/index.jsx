  ...
  return (
    <>
      <IndexMain
        ...
        // head table content
        addRowURL={getPermissions("social", "create")?config.routes.add:false}
        // head filters table
        filters={filters}
        // columns table content
        editRowURL={getPermissions("social", "update")?config.routes.edit:false}
         // showRowURL={config.routes.show}
         deleteRow={{ status: getPermissions("social", "delete"), deleteRecord }}
         action={(getPermissions("social", "delete")||getPermissions("social", "update"))||false}

         generalCols={generateCols([
   
          {
            key: "key",
            content: (text, record) =><div>{text.replace('_'," ")}</div>
          },
          
          
          {
            key: "value",
            content: (text, record) =><a href={text} target="_blank">{text}</a>
          }
