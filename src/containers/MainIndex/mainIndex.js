...
const Countries = ({
  ...
  ////////// *********************** /////////
  const SelectContent = ({ placeholder, key, data, multiple }, index) => {
    return (
      <Select
        mode={multiple}
        style={{ width: 200 }}
        showSearch
        value={multiple ? undefined : filtersParams[key]}
        onChange={(e) => {
          if (e) {
            setFiltersParams((old) => ({
              ...old,
  ...
  const fetchData = (url, { method, params: paraRenamed }) => {
    const params = {
      skip: start,
      ...paraRenamed,
      ...filtersParams,
    ...
    axios[method](url, {
      ...
      .then((res) => {
        if (url === "withdraws") {
          res.data.data = res.data.data.map((e) => ({ ...e, key: e.id }));
        }
        setData(res.data);
        ...
        {attachExport && (
          <Button
            ...
            onClick={() => {
              ...
              let searchParams = new URLSearchParams(allParams);
              window
                .open(
                  `${URL}/${indexRequest?.[1]}/export${
...
//           {el.type == "search" &&
//             filters.map((ele) =>
//               (ele?.hide?.hiddenFilters?.includes?.(el.uniq) &&
//                 ele?.hide?.onValue == filtersParams?.[ele?.key]) ||
//               !ele?.hide?.onValue ||
//               !ele?.hide?.hiddenFilters
//                 ? ""
//                 : SearchComponent(el)
//             )}
//         </>
